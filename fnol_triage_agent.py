...
def _fnol_register_path():
    return os.getenv("FNOL_REGISTER_PATH", "./fnol_register_sample.xlsx")
...
def build_claim_snapshot(fnol_fields: dict, fnol_id: str,
    ...
    policy = None
    for r in rows:
        if str(r.get("policy_id", "")).strip() == policy_number.strip():
            policy = r
            break
    if not policy:
        for r in rows:
    ...
    if incident_date and policy_expiry and incident_date > policy_expiry:
        expiry_warning = (
            f"Policy {policy['policy_id']} expired "
...
def check_claims_history(fnol_fields: dict, policy_row: dict) -> dict:
    ...
    member_id     = str(fnol_fields.get("member_id") or "").strip().upper()
    incident_date = _safe_date(fnol_fields.get("incident_date_time"))
...
def evaluate_escalation(fnol_fields: dict, policy_row: dict,
    ...
    """
    Evaluates all 10 escalation conditions.
    Returns escalation dict with triggered_conditions list.
    """
    triggered     = []
    incident_type = str(fnol_fields.get("incident_type", "")).strip().upper().replace(" ", "_")
...
def run_triage_agent(intake_state: dict, fnol_id: str,
                      fnol_received_at: str = None) -> FNOLTriageState:
    """
