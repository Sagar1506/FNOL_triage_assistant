...
def extract_pages_text(pdf_path: str) -> list[str]:
    """Extract text from every page. Returns list indexed from 0 = page 1."""
...
def document_intake_node(state: FNOLIntakeState) -> FNOLIntakeState:
    ...
    Steps:
      ...
      4. Identify document type per page (GPT-4o mini).
      5. Extract structured fields per document.
      ...
      9. Build extracted summary and return updated state.
    """
    try:
        ...
        fnol_fields = {}
        for i, doc in enumerate(documents_identified):
            dt = doc["doc_type"]
            if dt == "UNKNOWN":
                continue
            page_text = pages_text[doc["page"] - 1]
            fields = extract_document_fields(page_text, dt)
            documents_identified[i]["fields"] = fields
        ...
        # Step 10: plain-language claimant summary (GPT-4o mini)
...
def run_intake_agent(pdf_path: str) -> FNOLIntakeState:
    """
