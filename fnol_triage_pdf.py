...
def _render_fraud_signals(state: dict) -> list:
    """Section 9 — Elevated Review Indicators (fraud signal agent output)."""
    items = [_sec_header("9", "Elevated Review Indicators"), sp(4)]
    fraud = state.get("fraud_signals", {})

    if not fraud:
        items.append(p("Elevated review indicator data is not available for this FNOL.", "small"))
        items.append(sp(6))
        return items
    ...
    BAND_STYLE = {
        "LOW":    (GRN_BG, TEAL,  "LOW"),
        "MEDIUM": (AMB_BG, AMBER, "MEDIUM"),
        "HIGH":   (RED_BG, RED,   "HIGH"),
    }
    bg, fg, label = BAND_STYLE.get(band, (LT_GRAY, GRAY, band))

    items.append(p(
        "This section reports data-grounded indicators for adjuster and, where "
        "warranted, SIU (Special Investigation Unit) review. It is not a "
        "determination that this claim is fraudulent.", "small"
    ))
    items.append(sp(4))

    band_tbl = Table(
        [[Paragraph(
            f"<b>Band: {label}</b>  &nbsp;&nbsp;  Score: {score}<br/>"
            f"{fraud.get('adjuster_summary', '')}",
    ...
    items.append(band_tbl)
    items.append(sp(6))

    hdr = [
        Paragraph("<b><font color='white'>Indicator</font></b>",
                  ParagraphStyle("h", fontName="Helvetica-Bold", fontSize=9, textColor=WHITE)),
        Paragraph("<b><font color='white'>Type</font></b>",
                  ParagraphStyle("h", fontName="Helvetica-Bold", fontSize=9, textColor=WHITE)),
        Paragraph("<b><font color='white'>Status</font></b>",
                  ParagraphStyle("h", fontName="Helvetica-Bold", fontSize=9, textColor=WHITE)),
        Paragraph("<b><font color='white'>Reason</font></b>",
                  ParagraphStyle("h", fontName="Helvetica-Bold", fontSize=9, textColor=WHITE)),
    ]
    rows = [hdr]

    LLM_CODES = {"NARRATIVE_COHERENCE", "KEY_STATUS_CONCERN", "DAMAGE_SEVERITY_PLAUSIBILITY"}
    for s in fraud.get("signals", []):
        sig_type = "LLM-judged" if s.get("code") in LLM_CODES else "Deterministic"
        triggered = s.get("triggered")
        status_para = (
            Paragraph("<b>TRIGGERED</b>",
                      ParagraphStyle("tr", fontName="Helvetica-Bold", fontSize=9, textColor=RED))
            if triggered else
            Paragraph("Not triggered",
                      ParagraphStyle("nt", fontName="Helvetica", fontSize=9, textColor=TEAL))
        )
        rows.append([
            p(s.get("label", s.get("code", "")), "body"),
            p(sig_type, "small"),
            status_para,
            p(s.get("reason", ""), "small"),
        ])

    if len(rows) == 1:
        rows.append([p("No indicator data available.", "small"), p(""), p(""), p("")])

    items.append(_tbl(rows, [W*0.24, W*0.14, W*0.12, W*0.50], [
        ("BACKGROUND", (0,0), (-1,0), NAVY),
    ]))
    items.append(sp(6))
    return items
...
def render_triage_pdf(state: dict, output_path: str):
    """
    Render the complete 9-section FNOL Triage Summary PDF.
    ...
    story += _render_fraud_signals(state)
